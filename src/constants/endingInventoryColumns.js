...
export const ENDING_INVENTORY_DEFAULT_COLUMNS = [
    ...
    { title: 'الاجمالي', dataIndex: 'الاجمالي', key: 'الاجمالي', width: 100, align: 'center', render: v => formatMoney(v) },
    ...
    { title: 'عمر الصنف', dataIndex: 'عمر الصنف', key: 'عمر الصنف', width: 80, align: 'center', render: v => v ? `${v} يوم` : '-' },
    ...
    { title: 'فائض المخزون', dataIndex: 'excessInventory', key: 'excessInventory', width: 100, align: 'center', render: (_, record) => {
        const qty = parseFloat(record['الكمية']) || 0;
        const excessRatio = parseFloat(record['نسبة الفائض']) || 0;
        const excessInventory = qty * (excessRatio / 100);
        return formatQuantity(excessInventory);
    }},
    { title: 'قيمة فائض المخزون', dataIndex: 'valueExcessInventory', key: 'valueExcessInventory_1', width: 100, align: 'center', render: (_, record) => {
        const qty = parseFloat(record['الكمية']) || 0;
        const excessRatio = parseFloat(record['نسبة الفائض']) || 0;
    ...
    { title: 'قيمة معد للارجاع', dataIndex: 'valueReturns', key: 'valueReturns_1', width: 100, align: 'center', render: (_, record) => {
    ...
    { title: 'قيمة الاحتياج', dataIndex: 'valueNeed', key: 'valueNeed', width: 100, align: 'center', render: (_, record) => {
        const need = parseFloat(record['الاحتياج']) || 0;
        const unitPrice = parseFloat(record['الافرادي']) || 0;
        return formatMoney(need * unitPrice);
    }},
    { title: 'نسبة الفائض', dataIndex: 'نسبة الفائض', key: 'نسبة الفائض', width: 80, align: 'center', render: v => `${parseFloat(v) || 0}%` },
    ...
    { title: 'بيان الحالة', dataIndex: 'بيان الحالة', key: 'بيان الحالة', width: 120, align: 'right', render: v => v },
    { title: 'البيان', dataIndex: 'البيان', key: 'البيان', width: 120, align: 'right', render: v => v },
    { title: 'قيمة مخزون مثالي', dataIndex: 'قيمة مخزون مثالي', key: 'قيمة مخزون مثالي', width: 100, align: 'center', render: (_, record) => {
        const itemAge = parseFloat(record['عمر الصنف']) || 0;
        const qty = parseFloat(record['الكمية']) || 0;
        const unitPrice = parseFloat(record['الافرادي']) || 0;
    ...
    { title: 'قيمة فائض المخزون', dataIndex: 'valueExcessInventory', key: 'valueExcessInventory_2', width: 100, align: 'center', render: (_, record) => {
        const qty = parseFloat(record['الكمية']) || 0;
        const excessRatio = parseFloat(record['نسبة الفائض']) || 0;
        const unitPrice = parseFloat(record['الافرادي']) || 0;
        const excessInventory = qty * (excessRatio / 100);
    ...
    { title: 'قيمة معد للارجاع', dataIndex: 'valueReturns', key: 'valueReturns_2', width: 100, align: 'center', render: (_, record) => {
        const returns = parseFloat(record['معد للارجاع']) || 0;
        const unitPrice = parseFloat(record['الافرادي']) || 0;
        return formatMoney(returns * unitPrice);
    }},
    { title: 'قيمة صنف جديد', dataIndex: 'valueNewItem', key: 'valueNewItem', width: 100, align: 'center', render: (_, record) => {
        const itemAge = parseFloat(record['عمر الصنف']) || 0;
        const qty = parseFloat(record['الكمية']) || 0;
        const unitPrice = parseFloat(record['الافرادي']) || 0;
        return itemAge <= 90 ? formatMoney(qty * unitPrice) : formatMoney(0);
    }},
    { title: 'قيمة الاحتياج', dataIndex: 'valueNeed', key: 'valueNeed_2', width: 100, align: 'center', render: (_, record) => {
        const need = parseFloat(record['الاحتياج']) || 0;
        const unitPrice = parseFloat(record['الافرادي']) || 0;
        return formatMoney(need * unitPrice);
    }},
