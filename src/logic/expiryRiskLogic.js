...
export const calculateExpiryRiskForecast = (salesData, inventoryData) => {
    ...
    // Convert values and add sequential numbering
    riskForecast.forEach((item, index) => {
        item['م'] = index + 1;

        // التقريب للأسفل دائماً حسب طلب المستخدم (Floor rounding)
        item['معدل البيع اليومي'] = Math.floor(item['معدل البيع اليومي'].toNumber());
        item['الكمية المتوقعة للبيع'] = Math.floor(item['الكمية المتوقعة للبيع'].toNumber());

        // الخطر المتوقع = الكمية الحالية - المتوقع بيعه (بحد أدنى صفر)
        const currentQty = item['الكمية الحالية'].toNumber();
        const expectedSale = item['الكمية المتوقعة للبيع'];
        const risk = Math.max(0, currentQty - expectedSale);

        item['الخطر المتوقع'] = risk;

        // نسبة الخطر = ناتج قسمة المتوقع بيعه / الكمية الحالية (حسب طلب المستخدم)
        item['نسبة الخطر %'] = currentQty > 0 ? Math.round((expectedSale / currentQty) * 100) : 0;

        item['الكمية الحالية'] = currentQty;
    });
